...
export function Signup() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (success) {
      ...
      setError('Unable to register. Email might already exist.');
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div className="max-w-md w-full mx-4 relative z-10">
        ...
        >
          ...
          <h1 className="text-5xl mb-3">Create Account</h1>
          <p className="text-xl text-muted-foreground">Sign up to start your watchlist journey</p>
        </motion.div>
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {error && <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/50 text-destructive text-sm">{error}</div>}
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-muted-foreground">
              ...
              <button onClick={() => navigate('/login')} className="text-accent hover:text-accent/80 transition-colors">
                Sign in
              </button>
            </p>
          </div>
